...
class PgSqlHandler implements DbServerHandlerInterface {
  ...
  public function listTables() {
    $result = \Drupal::database()->query("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' ORDER BY table_name",
  ...
  public function optimizeTable($table_name) {
    try {
      \Drupal::database()->query("VACUUM ANALYZE {$table_name}")->execute();
    }
    catch (\Exception $e) {
